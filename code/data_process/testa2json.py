...
seq_root = '../data/track_3/3_test_imagesa'
label_file = '../data/track_3/3_testa_user.csv'
# save_image_root = './visualization'
#
# mkdirs(save_image_root)
save_json_root = '../user_data/tmp_data/3test.json'
...
for i in range(len(rows)-1):
    image_file_name = rows[i+1][0]
    # image_file_name_visual = image_file_name.replace('2_images', 'visualization')
    image_file = cv2.imread(os.path.join(seq_root, image_file_name).replace('3_test_imagesa/3_test_imagesa', '3_test_imagesa'))
