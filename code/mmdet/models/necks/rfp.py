...
class RFP_BFP(FPN):
    ...
    def __init__(self,
        ...
        self.rfp_weight = nn.Conv2d(
            ...
            bias=True)
        # bfp
        self.bfp_in_channels = 256
        self.bfp_num_levels = 5
        self.bfp_refine_level = 2
        self.bfp_refine = NonLocal2d(
            self.bfp_in_channels,
            reduction=1,
            use_scale=False,
            conv_cfg=None,
            norm_cfg=None)
    ...
    def forward(self, inputs):
        ...
        bfp_feats = []
        gather_size = x[self.bfp_refine_level].size()[2:]
        for i in range(self.bfp_num_levels):
            if i < self.bfp_refine_level:
                gathered = F.adaptive_max_pool2d(
                    x[i], output_size=gather_size)
            else:
                gathered = F.interpolate(
                    x[i], size=gather_size, mode='nearest')
            bfp_feats.append(gathered)

        bsf = sum(bfp_feats) / len(bfp_feats)

        # step 2: refine gathered features
        bsf = self.bfp_refine(bsf)
